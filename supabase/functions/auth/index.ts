...
import { corsHeaders, json, err, checkRateLimit } from '../_shared/auth.ts';
...
Deno.serve(async (req: Request) => {
  ...
  // Rate limit by IP — prevents brute-force token probing
  const ip = req.headers.get('CF-Connecting-IP') ?? req.headers.get('X-Forwarded-For') ?? 'unknown';
  const rateLimited = await checkRateLimit(supabase, ip, '/auth');
