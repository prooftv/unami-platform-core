...
import { getPreferencePersistence, type PreferenceKey, type PreferencePersistence, type PreferenceValueMap } from './preferences-config';

async function persistByMode(mode: PreferencePersistence, key: string, value: string): Promise<void> {
  switch (mode) {
    case 'none': return;
    case 'client-cookie': setClientCookie(key, value); return;
    case 'server-cookie': await setValueToCookie(key, value); return;
    case 'localStorage': setLocalStorageValue(key, value); return;
  }
}
