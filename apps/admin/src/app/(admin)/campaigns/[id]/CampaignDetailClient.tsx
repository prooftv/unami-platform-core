...
export function CampaignDetailClient({ campaign, transactions, session }: Props) {
  ...
  const totalSpent = transactions.filter((t) => t.transactionType === 'spend' && t.status === 'completed')
      ...
      <div className="flex items-start justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">{campaign.category} · {campaign.sponsor?.displayName ?? 'No sponsor'}</p>
          </div>
        </div>
