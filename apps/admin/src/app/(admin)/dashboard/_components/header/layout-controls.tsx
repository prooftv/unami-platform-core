      ...
      <PopoverContent align="end" sideOffset={8} className="w-80 p-4">
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <ToggleRow value={values.theme_mode} onValueChange={set('theme_mode')}
              options={[{ value: 'light', label: 'Light' }, { value: 'dark', label: 'Dark' }, { value: 'system', label: 'System' }]} />
          </div>

          <div className="space-y-2">
            <Label className="text-xs">Page Layout</Label>
            <ToggleRow value={values.content_layout} onValueChange={set('content_layout')}
              options={[{ value: 'centered', label: 'Centered' }, { value: 'full-width', label: 'Full Width' }]} />
          </div>

          <div className="space-y-2">
            <Label className="text-xs">Navbar</Label>
            <ToggleRow value={values.navbar_style} onValueChange={set('navbar_style')}
              options={[{ value: 'sticky', label: 'Sticky' }, { value: 'scroll', label: 'Scroll' }]} />
          </div>

          <div className="space-y-2">
            <Label className="text-xs">Sidebar Style</Label>
            <ToggleRow value={values.sidebar_variant} onValueChange={set('sidebar_variant')}
          ...
          <div className="space-y-2">
            ...
            <ToggleRow value={values.sidebar_collapsible} onValueChange={set('sidebar_collapsible')}
              options={[{ value: 'icon', label: 'Icon' }, { value: 'offcanvas', label: 'OffCanvas' }]} />
          </div>

          <Button variant="outline" size="sm" className="w-full" onClick={resetPreferences}>
            Restore Defaults
          </Button>
        </div>
      </PopoverContent>
