...
import type { MomentWithSponsor } from '@moments/api';
...
export function EditMomentClient({ moment }: Props) {
  ...
  async function handleSchedule() {
    ...
    if (moment.status !== 'draft') {
      setError('Only draft moments can be scheduled');
      return;
    }
    setScheduling(true);
    setError(null);
    try {
      const token = await getToken();
      const api = createApiClient({ baseUrl: process.env.NEXT_PUBLIC_SUPABASE_URL! + '/functions/v1', token });
      await api.moments.schedule(moment.id, {
    ...
    <div className="max-w-2xl space-y-6">
      ...
      <form onSubmit={handleSave} className="space-y-8">
        ...
        <div className="space-y-4">
          ...
          <div className="flex flex-col gap-3">
            {([
              ...
              { field: 'isSponsored', label: 'Sponsored content', value: form.isSponsored },
        ...
        <div className="flex items-center justify-end gap-2 pt-2 border-t">
          <Button type="button" variant="outline" onClick={() => router.push(`/moments/${moment.id}`)}>
            Cancel
          </Button>
